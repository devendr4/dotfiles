...
case "$1" in
	...
	"maim")
		case "$2" in
            fullToFile )
                FILENAME=$SCREENSHOT_DIR/Screenshot_$(date +%s).png
                maim $FILENAME
                $ACTIONS_PATH $FILENAME
                ;;
			fullToClip )
				maim -o | xclip -selection clipboard -t image/png
            ...
            selectToFile )
                ...
                ;;
			selectToClip )
				maim -so | xclip -selection clipboard -t image/png
                $ACTIONS_PATH clipboard
				;;
            windowToFile )
                FILENAME=$SCREENSHOT_DIR/$(date +%s).png
                ...
                ;;
			windowToClip )
				maim -io $(xdotool getactivewindow) | xclip -selection clipboard -t image/png
                $ACTIONS_PATH clipboard
				;;
			*)
				send_error "$2 is not a supported action for $1"
